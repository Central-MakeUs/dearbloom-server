...
public class SharedArtworkFacade {
     ...
     * 같은 작품을 여러 참여자가 담았어도 <b>작품 하나로 합쳐</b> 내려가며, 정렬은 좋아요 많은 순 →
     * 먼저 담긴 순이다.
     */
    ...
    public SharedArtworkPageResponse getPage(Customer customer, Long sharedBoardId) {
        ...
        List<SharedArtworkSummaryResponse> sharedArtworks = toSummaries(
                sharedArtworkQueryService.getDistinctBySharedBoard(sharedBoard),
                sharedArtworkQueryService.getLikedArtworkIds(sharedBoard, customer));
    ...
    public List<SavedArtworkIsSharedResponse> getSavedArtworks(Customer customer, Long sharedBoardId) {
        ...
        Set<Long> sharedArtworkIds = sharedArtworkQueryService.getMySharedArtworkIds(sharedBoard, customer);
        ...
        return artworkQueryService.getSummaries(savedArtworks, savedArtworkIds).stream()
                .map(summary -> new SavedArtworkIsSharedResponse(
                        summary, sharedArtworkIds.contains(summary.artworkId())))
                // 공유 중인 작품을 앞으로. 동률은 stable sort 라 저장 목록 순서가 유지된다.
     ...
     * 다른 참여자가 같은 작품을 담고 있어도 그 사람 것은 그대로 남는다.
     */
    @Transactional
    ...
    /**
     * 대표 공유작품 목록을 카드로 변환. 가격·대표 이미지는 작품 도메인의 배치 조회를 재사용하고
        ...
        return IntStream.range(0, summaries.size())
                .mapToObj(index -> {
                    ...
                    return new SharedArtworkSummaryResponse(
                            sharedArtworks.get(index).getSharedArtworkId(),
                            summary.title(),
                            summary.lowestPrice(),
                            ...
                            likedArtworkIds.contains(summary.artworkId()));
                })
                .toList();
    }
}
